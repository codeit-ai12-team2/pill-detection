...
#     희귀 클래스(데이터 수가 적은 클래스)부터 우선 배정하고, 부족한 val 비율은
...
#     if missing_in_train:
#         print(f" train에 여전히 없는 클래스: {[class_names[i] for i in missing_in_train]}")
...
#         print(f" 여전히 train에 없는 클래스 (데이터가 1개뿐이라 보정 불가): {[class_names[i] for i in still_missing_train]}")
#     if not overlap_final and not still_missing_train:
#         print(f" train은 {len(all_classes)}개 클래스 전부 확보, 데이터 중복 없음 (val 일부 클래스 누락은 데이터 한계로 감수)")

#     return combo_to_files, train_combos, val_combos


# def apply_resplit(combo_to_files, train_combos, val_combos):
...
# def oversample_rare_classes(target_min_count: int = 20):
...
# def main():
#     parser = argparse.ArgumentParser(description="Train/Val K-코드 기반 데이터 누수 점검 및 재분리")
