...
def extract_archives(tar_dir: Path, target_dir: Path) -> None:
    ...
    for tar_path in tqdm(tar_paths, desc=f"Extracting tar ({tar_dir.name})\t"):
        ...
        for zip_name, parts in tqdm(
            parts_by_zip.items(),
            desc=f"Merging & unzipping {tar_path.name}\t",
            leave=False,
        ):
            parts.sort(
                key=lambda p: int(ZIP_PART_PATTERN.match(p.name).group("offset"))
            )
...
def remove_invalid_categories(
    labels_dir: Path, images_dir: Path, valid_category_ids: set[int]
) -> None:
...
def _normalize_annotation(
    json_path: Path, category_id: int, category_name: str
) -> None:
